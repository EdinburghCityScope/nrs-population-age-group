...
const csvFilename = 'population-age-group.csv';
...
function interrogateSPARQL(zones) {
    ...
    _.forEach(_.chunk(zones, chunk_size), (zone_chunk) => {
        queries.push(`
            ...
            SELECT ?year ?zone ?age ?gender ?measure ?value
            WHERE {
                ?s qb:dataSet data:population-estimate-groups ;
                   qb:measureType ?m ;
                   ?m ?value ;
                   dim:gender ?g ;
                   dim:populationGroup ?a ;
                   ldim:refArea ?z ;
                   ldim:refPeriod ?y .
                ?g rdfs:label ?gender .
                ?z rdfs:label ?zone .
        ...
        queries.push(`
            ...
            SELECT ?year ?zone ?age ?gender ?measure ?value
            WHERE {
                ?s qb:dataSet data:population-estimates ;
                   qb:measureType ?m ;
                   ?m ?value ;
                   dim:gender ?g ;
                   dim:age ?a ;
